...
class DConv(nn.Module):
    ...
    def __init__(self, channels: int, compress: float = 4, depth: int = 2, init: float = 1e-4,
                 norm=False, attn=False, heads=4, ndecay=4, lstm=False, gelu=True,
...
class My_Model_Time(nn.Module):
    ...
    def __init__(self,
        ...
        for index in range(depth):
            ...
            encode = []
            encode += [
                nn.Conv1d(in_channels, channels, kernel_size, stride, padding=kernel_size // 2),
                norm_fn(channels),
                act2(),
    ...
    def forward(self, mix):
        ...
        for encode in self.encoder:
            ...
            # input(f"인코딩 후 {x.shape}")
        # print("------------------------------------------\n인코딩끝\n------------------------------------------")
        if self.lstm:
            x = self.lstm(x)
            print(f"lstm하고 후 {x.shape}")
        for decode in self.decoder:
            skip = saved.pop(-1)
            skip = center_trim(skip, x)
            x = decode(x + skip)
            # input(f"디코딩 후 {x.shape}")
        # if self.resample:
