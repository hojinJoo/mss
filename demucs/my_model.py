...
class HEncLayer(nn.Module):
    def __init__(self, chin, chout, kernel_size=8, stride=4, norm_groups=1, empty=False,
        ...
        if freq:
            ...
            pad = [pad, 0]
            klass = nn.Conv2d
        self.conv = klass(chin, chout, 9, 1, 4)
        if self.empty:
            return
...
class MyModel(nn.Module):
    ...
    def _spec(self, x):
        ...
        nfft = self.nfft
        x0 = x  # noqa
        print(f"hl {hl} nfft {nfft} x.shape {x.shape}")
        if self.hybrid:
    ...
    def forward(self, mix):
        x = mix
        length = x.shape[-1]
        print(f"before spec {x.shape}")
        z = self._spec(mix)
        print(f"after spec {z.shape}")
        mag = self._magnitude(z)
        print(f"after mag {mag.shape}")
        x = mag
        print(f"mix shape: {mix.shape} x shape: {x.shape}")
        B, C, Fq, T = x.shape
        ...
        for idx, encode in enumerate(self.encoder):
            ...
            if self.hybrid and idx < len(self.tencoder):
                ...
                print(f"xt 이전 사이즈 : {xt.size()}")
                xt = tenc(xt)
                print(f"xt 이후 사이즈 : {xt.size()}")
                if not tenc.empty:
                    ...
                    inject = xt
            print(f"전 x 사이즈 : {x.size()}")    
            x = encode(x, inject)
            input(f"후 x 사이즈 : {x.size()}")
        ...
        for idx, decode in enumerate(self.decoder):
            ...
            if self.hybrid and idx >= offset:
                ...
                if tdec.empty:
                    ...
                    skip = saved_t.pop(-1)
                    #TODO: 여기가 문제!
                    print(f"xt : {xt.size()} skip : {skip.size()}")
